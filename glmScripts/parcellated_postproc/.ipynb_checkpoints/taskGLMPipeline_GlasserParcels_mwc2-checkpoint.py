...
def runGroupTaskGLM(nuisModel='24pXaCompCorXVolterra', taskModel='FIR', nproc=10, byRun=False, zscore=False):
    scount = 1
    for subj in subjNums:
        ...
        if byRun:
            ...
            for task in taskNames:
                taskGLM(subj, task, taskModel=taskModel, nuisModel=nuisModel, nproc=nproc, zscore=zscore)
...
def taskGLM(subj, task, taskModel='canonical', nuisModel='24pXaCompCorXVolterra', nproc=8, zscore=False):
    """
    This function runs a task-based GLM on a single subject
    Will only regress out task-timing, using either a canonical HRF model or FIR model
    ...
    try:
        if taskModel=='FIR' or taskModel=='FIR_plus_canonical':
            h5f.create_dataset(outname1,data=residual_ts)
        else:
            h5f.create_dataset(outname1,data=residual_ts)
            h5f.create_dataset(outname2,data=betas)
