...
datadir = basedir + 'TaskFCActflow/data/hcppreprocessedmsmall/'
...
outputdir = datadir + 'parcellated_postproc/'
parcelCIFTIFile_directory='/projects/f_mc1689_1/TaskFCActflow/docs/scripts/final_analyses/ActflowToolbox/dependencies/ColeAnticevicNetPartition/'
...
taskRuns_Identifier = {'tfMRI_EMOTION':['tfMRI_EMOTION_RL','tfMRI_EMOTION_LR'],
                     'tfMRI_GAMBLING':['tfMRI_GAMBLING_RL','tfMRI_GAMBLING_LR'],
                     'tfMRI_LANGUAGE':['tfMRI_LANGUAGE_RL','tfMRI_LANGUAGE_LR'],
                     'tfMRI_MOTOR':['tfMRI_MOTOR_RL','tfMRI_MOTOR_LR'],
                     'tfMRI_RELATIONAL':['tfMRI_RELATIONAL_RL','tfMRI_RELATIONAL_LR'],
                     'tfMRI_SOCIAL':['tfMRI_SOCIAL_RL','tfMRI_SOCIAL_LR'],
                     'tfMRI_WM':['tfMRI_WM_RL','tfMRI_WM_LR']}

def runGroupTaskGLM(nuisModel='24pXaCompCorXVolterra', taskModel='FIR', nproc=10, byRun=False, zscore=False, verbose=False):
    scount = 1
    for subj in subjNums:
        print('Running nuisance regression and task GLM for subject', subj, '|', scount, '/', len(subjNums), '... model:', taskModel, '... zscore:', zscore)
        if byRun:
            for task in taskNames:
                taskGLM_byRun(subj, task, taskModel=taskModel, nuisModel=nuisModel, nproc=nproc, zscore=zscore)
        else:
            for task in taskNames:
                taskGLM(subj, task, taskModel=taskModel, nuisModel=nuisModel, nproc=nproc, zscore=zscore, verbose=verbose)
        
        scount += 1

def runSingleSubjTaskGLM(subj, nuisModel='24pXaCompCorXVolterra', taskModel='FIR', nproc=10, byRun=False, zscore=False, verbose=False, nuisance_reg_simultaneous=False):
    if byRun:
        for task in taskNames:
            taskGLM_byRun(subj, task, taskModel=taskModel, nuisModel=nuisModel, nproc=nproc, zscore=zscore)
    else:
        for task in taskNames:
            taskGLM(subj, task, taskModel=taskModel, nuisModel=nuisModel, nproc=nproc, zscore=zscore, verbose=verbose, nuisance_reg_simultaneous=nuisance_reg_simultaneous)
...
def taskGLM_byRun(subj, task, taskModel='canonical', nuisModel='24pXaCompCorXVolterra', nproc=8, zscore=False):
    """
    ...
    h5f = h5py.File(outputdir + subj + '_glmPlusNuisRegOutput_data.h5','a')
    ...
    if taskModel is 'FIR_plus_canonical':
        X1 = loadTaskTiming(subj, task, taskModel='FIR', nRegsFIR=25)['taskRegressors']
        X2 = loadTaskTiming(subj, task, taskModel='canonical', nRegsFIR=25)['taskRegressors']
        taskRegs = np.hstack((X1, X2))
    else:
        X = loadTaskTiming(subj, task, taskModel=taskModel, nRegsFIR=25)
        taskRegs = X['taskRegressors']
    num_task_regs=np.shape(taskRegs)[1]
...
def taskGLM(subj, task, taskModel='canonical', nuisModel='24pXaCompCorXVolterra', nproc=8, zscore=False, nuisance_reg_simultaneous=False, verbose=False):
    """
    This function runs a task-based GLM on a single subject
    Will only regress out task-timing, using either a canonical HRF model or FIR model
    ...
    if nuisance_reg_simultaneous:
        taskRuns=taskRuns_Identifier[task]
        run1 = _load_parcellated_timeseries(subj, taskRuns[0], parcelCIFTIFile_directory, rawdatadir_base)
        run2 = _load_parcellated_timeseries(subj, taskRuns[1], parcelCIFTIFile_directory, rawdatadir_base)
        #Drop skipped frames
        run1 = run1[:,framesToSkip:]
        run2 = run2[:,framesToSkip:]
    else:
        h5f = h5py.File(outputdir + subj + '_glmPlusNuisRegOutput_data.h5','r')
        run1 = h5f[task+'_RL']['nuisanceReg_resid_'+nuisModel][:].copy()
        run2 = h5f[task+'_LR']['nuisanceReg_resid_'+nuisModel][:].copy()
        h5f.close()      
    ...
    if nuisance_reg_simultaneous:
        if verbose:
            print('Getting nuisance regressors')
        nuisanceregs_run1=nrp._nuisanceRegression(subj, taskRuns[0], data, outputdir,  model='24pXaCompCorXVolterra', spikeReg=False, zscore=False, nproc=nproc, just_regressors_for_taskGLM=True)
        nuisanceregs_run2=nrp._nuisanceRegression(subj, taskRuns[1], data, outputdir,  model='24pXaCompCorXVolterra', spikeReg=False, zscore=False, nproc=nproc, just_regressors_for_taskGLM=True)
        nuisanceregs = np.vstack((nuisanceregs_run1, nuisanceregs_run2))
        num_nuisanceregs=np.shape(nuisanceregs)[1]
        taskRegs = np.hstack((taskRegs,nuisanceregs))
    ...
    outputfile=outputdir + subj + '_glmPlusNuisRegOutput_data.h5'
    if verbose:
    ...
    if nuisance_reg_simultaneous:
        outname3 = 'taskRegression/' + task + '_' + nuisModel + '_taskReg_nuisbetas_' + taskModel + zscore_str
    try:
        h5f.create_dataset(outname1,data=residual_ts)
        h5f.create_dataset(outname2,data=betas[:,0:(num_task_regs+1)])
        if nuisance_reg_simultaneous:
            h5f.create_dataset(outname3,data=betas[:,(num_task_regs+1):(num_task_regs+num_nuisanceregs+1)])
    except:
        del h5f[outname1]
        del h5f[outname2]
        if nuisance_reg_simultaneous:
...
def taskGLM_onRest(subj, taskModel='canonical', nuisModel='24pXaCompCorXVolterra', nproc=8):
    """
    This function runs a task-based GLM on a single subject
    Will only regress out task-timing, using either a canonical HRF model or FIR model
...
def _load_parcellated_timeseries(subj, run, parcelCIFTIFile_directory, rawdata_directory):
    
    #The data are loaded left-hemisphere then right-hemisphere
    
    #Setting the parcel files to be the 360 Glasser2016 cortical parcels
    L_parcelCIFTIFile=parcelCIFTIFile_directory+'SeparateHemispheres/Q1-Q6_RelatedValidation210.L.CorticalAreas_dil_Final_Final_Areas_Group_Colors.32k_fs_LR.dlabel.nii'
    R_parcelCIFTIFile=parcelCIFTIFile_directory+'SeparateHemispheres/Q1-Q6_RelatedValidation210.R.CorticalAreas_dil_Final_Final_Areas_Group_Colors.32k_fs_LR.dlabel.nii'

    from random import randint
    randnum=randint(1000, 10000)
    L_parcelTSFilename=str(randnum) + str(subj)+'_Output_Atlas.L.Parcels.32k_fs_LR.ptseries.nii'
    R_parcelTSFilename=str(randnum) + str(subj)+'_Output_Atlas.R.Parcels.32k_fs_LR.ptseries.nii'

    #Set this to be your input fMRI data CIFTI file
    inputFile=rawdata_directory+'/'+subj+'/MNINonLinear/Results/'+run+'/'+run+'_Atlas_MSMAll.dtseries.nii'
