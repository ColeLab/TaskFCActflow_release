...
def runGroupTaskGLM(nuisModel='24pXaCompCorXVolterra', taskModel='FIR', nproc=10, byRun=False, zscore=False, taskResid=False, taskResid2=False, taskResidModel='canonical', taskResidModel2='FIR'):
    scount = 1
    for subj in subjNums:
        print('Running task GLM for subject', subj, '|', scount, '/', len(subjNums), '... model:', taskModel, '... zscore:', zscore)
        if byRun:
            for task in taskNames:
                taskGLM_byRun(subj, task, nuisModel=nuisModel, nproc=nproc, zscore=zscore)
        else:
            for task in taskNames:
                taskGLM(subj, task, taskModel=taskModel, nuisModel=nuisModel, nproc=nproc, zscore=zscore, taskResid=taskResid, taskResid2=taskResid2, taskResidModel=taskResidModel, taskResidModel2=taskResidModel2) 
        
        scount += 1

def runSingleSubjTaskGLM(subj, nuisModel='24pXaCompCorXVolterra', taskModel='FIR', nproc=10, byRun=False, zscore=False, verbose=False):
    if byRun:
        for task in taskNames:
            taskGLM_byRun(subj, task, taskModel=taskModel, nuisModel=nuisModel, zscore=zscore)
    else:
        for task in taskNames:
...
def taskGLM_byRun(subj, task, nuisModel='24pXaCompCorXVolterra', nproc=8, zscore=False):
    """
    ...
    h5f = h5py.File(outputdir + subj + '_glmOutput2_data.h5','a')
...
def taskGLM(subj, task, taskModel='canonical', nuisModel='24pXaCompCorXVolterra', nproc=8, zscore=False, taskResid=False, taskResid2=False, taskResidModel='canonical', taskResidModel2='FIR'):
    """
    This function runs a task-based GLM on a single subject
    Will only regress out task-timing, using either a canonical HRF model or FIR model
    ...
    if taskResidModel is 'FIR_plus_canonical':
        h5f = h5py.File(outputdir + subj + '_glmOutput_FIR_plus_canonical_data.h5','a')
    elif taskResid2:
        h5f = h5py.File(outputdir + subj + '_withTaskResid_glmOutput_data.h5','a')
    else:
        h5f = h5py.File(outputdir + subj + '_glmOutput_data.h5','r') 

    #Use residuals from previous task regression (to help rule out certain confounds)
    if taskResid or taskResid2:
        dataid = task + '_' + nuisModel + '_taskReg_resid_' + taskResidModel
        #Load in previous residualized data (via both canonical and FIR)
        if taskResid2:
            dataid = dataid + 'taskResidFrom' + taskResidModel2
        print(dataid)
        data = h5f['taskRegression'][dataid][:].copy()
    else:
        run1 = h5f[task+'_RL']['nuisanceReg_resid_'+nuisModel][:].copy()
        run2 = h5f[task+'_LR']['nuisanceReg_resid_'+nuisModel][:].copy()
    ...
    if taskResid:
        h5f = h5py.File(outputdir + subj + '_withTaskResid_glmOutput_data.h5','a')
        outname1 = 'taskRegression/' + task + '_' + nuisModel + '_taskReg_resid_' + taskModel + zscore_str + 'taskResidFrom' + taskResidModel
        outname2 = 'taskRegression/' + task + '_' + nuisModel + '_taskReg_betas_' + taskModel + zscore_str + 'taskResidFrom' + taskResidModel
    elif taskResid2:
        h5f = h5py.File(outputdir + subj + '_withTaskResid_glmOutput_data.h5','a')
        outname1 = 'taskRegression/' + task + '_' + nuisModel + '_taskReg_resid_' + taskModel + zscore_str + 'taskResidFrom' + taskResidModel + 'taskResid2From' + taskResidModel2
        outname2 = 'taskRegression/' + task + '_' + nuisModel + '_taskReg_betas_' + taskModel + zscore_str + 'taskResidFrom' + taskResidModel + 'taskResid2From' + taskResidModel2
        print(outname2)
    else:
