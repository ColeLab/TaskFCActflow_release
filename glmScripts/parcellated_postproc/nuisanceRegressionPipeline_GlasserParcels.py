...
datadir='/projects/f_mc1689_1/TaskFCActflow/data/'
...
outputdir = datadir + 'hcppreprocessedmsmall/parcellated_postproc/'
...
def step1_createNuisanceRegressors(nproc=5):
    ...
    **Note: Parameters in this function may need to be edited for project-specific purposes. Sections in which editing should NOT be done are noted
    ...
    def _createNuisanceRegressorsSubject(subj):
        ...
        maskdir = datadir + 'hcppreprocessedmsmall/masks/' + subj 
...
def step2_nuisanceRegression(subj_nums=subjNums, nproc=5, model='24pXaCompCorXVolterra', spikeReg=False, zscore=False, just_regressors_for_taskGLM=False):
    """
    ...
    for subj in subj_nums:
        ...
        for run in allRuns:
            ...
            ## Parcellate data
            data = _load_parcellated_timeseries(subj, run, parcelCIFTIFile_directory, rawdatadir_base)
            ## Load in data to be preprocessed - This needs to be a space x time 2d array
            #inputfile = datadir + 'parcellated_minimalpreproc/' + subj + '/' + subj + '_GlasserParcellated_' +  run + '_LR.csv'
            # Load data
            ...
            _nuisanceRegression(subj, run, data, outputdir, model=model,spikeReg=spikeReg,zscore=zscore,nproc=nproc, just_regressors_for_taskGLM=just_regressors_for_taskGLM)


#########################################
# Functions that probably don't need to be edited

def _load_parcellated_timeseries(subj, run, parcelCIFTIFile_directory, rawdata_directory, cortex_only=True):
    
    LR_parcelCIFTIFile=parcelCIFTIFile_directory+ 'CortexSubcortex_ColeAnticevic_NetPartition_wSubcorGSR_parcels_LR.dlabel.nii'
    
    from random import randint
    randnum=randint(1000, 10000)
    LR_parcelTSFilename=str(randnum) + str(subj)+'_Output_Atlas.LR.Parcels.32k_fs_LR.ptseries.nii'
    ...
    LR_command='wb_command -cifti-parcellate ' + inputFile + ' ' + LR_parcelCIFTIFile + ' COLUMN ' + LR_parcelTSFilename + ' -method MEAN'
    os.system(LR_command)
    print(LR_command)

    # Load in parcellated data using nibabel
    lr_parcellated = np.squeeze(nib.load(LR_parcelTSFilename).get_data()).T
    #Remove temporary files
    os.system('rm '+LR_parcelTSFilename)
    
    #Flip hemispheres, since right hemisphere loaded first by default (but standard for partition is left-then-right)
    #flip_hemisphere_indices=np.concatenate([np.arange(180,360), np.arange(0,180)])
    #lr_parcellated=lr_parcellated[flip_hemisphere_indices]
    
    if cortex_only:
        lr_parcellated=lr_parcellated[0:360,:]

    return lr_parcellated
    

def _nuisanceRegression(subj, run, inputdata, outputdir,  model='24pXaCompCorXVolterra', spikeReg=False, zscore=False, nproc=8, just_regressors_for_taskGLM=False):
    """
    ...
    if just_regressors_for_taskGLM:
        ...
        outputfilename = outputdir + subj + '_glmPlusNuisRegOutput_data.h5'
